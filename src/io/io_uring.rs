...
pub struct InnerUringIO {
    pub ring: WrappedIOUring,
...
struct WritevState {
    ...
    total_len: usize,
    /// buffers to write
    bufs: Vec<Arc<turso_core::Buffer>>,
...
impl WritevState {
    fn new(file: &UringFile, pos: u64, linked: bool, bufs: Vec<Arc<turso_core::Buffer>>) -> Self {
        let file_id = file
            .id()
            .map(Fd::Fixed)
...
impl InnerUringIO {
    fn register_file(&mut self, fd: i32) -> Result<u32> {
        ...
        Err(turso_core::CompletionError::UringIOError(
...
impl WrappedIOUring {
    ...
    fn flush_overflow(&mut self) -> Result<()> {
        while !self.overflow.is_empty() {
            ...
            if available_space == 0 {
                ...
                return Err(turso_core::CompletionError::UringIOError("squeue full").into());
            }
    ...
    fn handle_writev_completion(&mut self, mut state: WritevState, user_data: u64, result: i32) {
        ...
        if written == 0 && state.remaining() > 0 {
            state.free_last_iov(&mut self.iov_pool);
            turso_completion_from_key(user_data).error(turso_core::CompletionError::ShortWrite);
            return;
        }
        ...
        match state.remaining() {
            0 => {
                ...
                turso_completion_from_key(user_data).complete(state.total_written as i32);
            }
            remaining => {
                tracing::trace!(
...
impl TursoIO for UringIO {
    fn open_file(&self, path: &str, flags: OpenFlags, direct: bool) -> Result<Arc<dyn File>> {
        ...
        if std::env::var(ENV_DISABLE_FILE_LOCK).is_err() {
    ...
    fn cancel(&self, completions: &[TursoCompletion]) -> Result<()> {
        let mut inner = self.inner.lock();
        for c in completions {
            c.abort();
            let e = io_uring::opcode::AsyncCancel::new(get_key_from_turso_completion(c.clone()))
                .build()
    ...
    fn register_fixed_buffer(&self, ptr: std::ptr::NonNull<u8>, len: usize) -> Result<u32> {
        ...
        let slot = inner.free_arenas.iter().position(|e| e.is_none()).ok_or(
            turso_core::CompletionError::UringIOError("no free fixed buffer slots"),
        )?;
        unsafe {
...
impl File for UringFile {
    ...
    fn pread(&self, pos: u64, c: TursoCompletion) -> Result<TursoCompletion> {
        let r = c.as_read();
        let read_e = {
            let buf = r.buf();
            ...
            with_fd!(self, |fd| {
                if let Some(idx) = buf.fixed_id() {
                    ...
                    io_uring::opcode::ReadFixed::new(fd, ptr, len as u32, idx as u16)
                        .offset(pos)
                        .build()
                        .user_data(get_key_from_turso_completion(c.clone()))
                } else {
    ...
    fn sync(&self, c: TursoCompletion) -> Result<TursoCompletion> {
        let mut io = self.io.lock();
        trace!("sync()");
    ...
    fn pwritev(
        ...
        io.ring.submit_writev(
            get_key_from_turso_completion(c.clone()),
            state,
            continue_chain,
        );
        Ok(c)
    ...
    fn truncate(&self, len: u64, c: TursoCompletion) -> Result<TursoCompletion> {
